...
import com.usa.reto3.reto3.service.ServiciosQuadbike;
...
public class QuadbikeWeb {
    ...
    @GetMapping("/{id}")
    public Optional<Quadbike> getQuadbike(@PathVariable("id") int IdQuadbike) {
        return servicios.getQuadbike(IdQuadbike);
    }

    @PostMapping("/save")
    @ResponseStatus(HttpStatus.CREATED)
    public Quadbike save(@RequestBody Quadbike quadbike) {
        return servicios.save(quadbike);
    }
    @PutMapping("/update")
    @ResponseStatus(HttpStatus.CREATED)
    ...
    public boolean delete(@PathVariable("id") int bikeId) {
        return servicios.deleteQuadbike(bikeId);
    } 
